...
现在来看下tagged_with('ruby', :exclude => true)

```
[6] pry(main)> Topic.tagged_with('ruby', :exclude => true)
  Topic Load (0.4ms)  SELECT `topics`.* FROM `topics` WHERE (topics.id NOT IN (SELECT taggings.taggable_id FROM taggings JOIN tags ON taggings.tag_id = tags.id AND (tags.name LIKE 'ruby') WHERE taggings.taggable_type = 'Topic'))
=> [#<Topic id: 5, title: "22222sss", created_at: "2014-01-06 15:02:15", updated_at: "2014-01-06 15:03:03">]
```
...
if options.delete(:exclude)
  ...
  # 关键语句,先查taggings表,taggings表 joins tags表,再用上面查找tags表的结果进行关联,而选择的数据则是topics.id不在taggings.taggable_id中的
...
关键语句在select topics.id not in ...这一行
